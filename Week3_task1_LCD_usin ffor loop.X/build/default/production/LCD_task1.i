...
unsigned char address[7]={0x82,0x83,0x84,0x85,0x86,0x87,0x88};
unsigned char address1[7]={0xCE,0xCD,0xCC,0xCB,0xCA,0xC9,0xC8};
void main()
{
    init();

    for(int i=0;i<=6;i++){
         LCD_command(address[i]);
        LCD_data(array[i]);
    }
    for(int i=0,c=6;i<=6,c>=0;i++,c--){
         LCD_command(address1[i]);
        LCD_data(array[c]);
    }
